...
it("should buy token successfully", async () => {
    ...
    expect(Invest.findOne).toHaveBeenCalledWith({
        where: {
            tokenId: mockedToken.id,
            userId: mockedUser.id,
        },
    });
    expect(Invest.create).not.toHaveBeenCalled();
    expect(Invest.update).toHaveBeenCalledWith(
        { count: 8 },
        {
            where: {
                tokenId: mockedToken.id,
